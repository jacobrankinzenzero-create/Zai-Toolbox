...
const AppCard = ({ title, description, icon: Icon, onClick, accentColor = "#ff8300" }) => (
  <div 
    onClick={onClick}
  ...
  >
    ...
    <div className="flex items-center gap-4 mb-4">
      <div 
        ...
        style={{ background: `linear-gradient(135deg, ${accentColor}, #d83b01)` }}
      >
    ...
    <div className="mt-6 flex items-center text-sm font-semibold transition-colors duration-200" style={{ color: accentColor }}>
      Launch Tool <ArrowRight className="ml-1 w-4 h-4 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300" />
    </div>
  </div>
);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-8">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 text-center max-w-md w-full">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Navigated to {currentRoute}</h2>
          <p className="text-gray-500 mb-6">This is a mock page transition to demonstrate routing.</p>
          <button 
            onClick={() => navigate('/')}
    ...
    <div className="min-h-screen bg-gray-50 font-sans selection:bg-orange-100 selection:text-orange-900">
      ...
      <header className="bg-white border-b border-gray-200 px-8 py-4 sticky top-0 z-30 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-black text-lg shadow-sm" style={{ background: 'linear-gradient(135deg, #ff8300, #ff4700)' }}>
            T
          </div>
          <div>
            <h1 className="font-bold text-xl tracking-tight text-gray-900">Toolbox</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-8 py-12">
        <div className="mb-10">
          ...
          <p className="text-gray-500">Select a tool below to start a new project or continue your work.</p>
        </div>
